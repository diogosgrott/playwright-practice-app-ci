...
test.describe('Form Layouts page', () => {
  ...
  test('input fields', async ({ page }, testInfo) => {
    ...
    await usingTheGridEmailInput.pressSequentially('test2@teste.com.br')

    //generic assertion
    const inputValue = await usingTheGridEmailInput.inputValue()
    expect(inputValue).toEqual('test2@teste.com.br')

    //locator assertion
    await expect(usingTheGridEmailInput).toHaveValue('test2@teste.com.br')
  })
  ...
  test.only('radio buttons', async ({ page }) => {
    ...
    // await expect(await usingTheGridEmailInput.getByRole('radio', { name: "Option 1" })).toBeChecked()
...
test('web tables', async ({ page }) => {
  ...
  for (let age of ages) {
    ...
    const rowsCount = await ageRows.count();

    if (rowsCount === 1) {
      const singleRowText = await ageRows.nth(0).textContent();
      if (singleRowText.includes('No data found')) {
        expect(singleRowText).toContain('No data found');
      } else {
        for (let row of await ageRows.all()) {
          const cellValue = await row.locator('td').last().textContent();
          expect(cellValue.trim()).toEqual(age); // trim() remove espaços extras
        }
      }
    } else {
      for (let row of await ageRows.all()) {
